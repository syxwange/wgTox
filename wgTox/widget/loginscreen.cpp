...
LoginScreen::LoginScreen(QString initialProfile, QWidget* parent)  : QDialog(parent)  , ui(new Ui::LoginScreen)  , quitShortcut{QKeySequence(Qt::CTRL + Qt::Key_Q), this}
{
    ui->setupUi(this);
...
void LoginScreen::onLogin()
{
    ...
    if (name.isEmpty()) {
        QMessageBox::critical(this, tr("Couldn't load profile"),tr("There is no selected profile.\n\n""You may want to create one."));
        return;
    }
