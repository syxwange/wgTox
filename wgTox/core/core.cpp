...
{
    tox_callback_friend_request(tox, onFriendRequest,nullptr);
    tox_callback_friend_message(tox, onFriendMessage, nullptr);
    tox_callback_friend_name(tox, onFriendNameChange, nullptr);
    tox_callback_friend_typing(tox, onFriendTypingChange, nullptr);
    tox_callback_friend_status_message(tox, onStatusMessageChanged, nullptr);
    tox_callback_friend_status(tox, onUserStatusChanged, nullptr);
    tox_callback_friend_connection_status(tox, onConnectionStatusChanged, nullptr);
    tox_callback_friend_read_receipt(tox, onReadReceiptCallback, nullptr);
   /* tox_callback_conference_message(tox, onGroupMessage);
    tox_callback_conference_peer_list_changed(tox, onGroupPeerListChange,nullptr);
    tox_callback_conference_peer_name(tox, onGroupPeerNameChange);
    tox_callback_conference_title(tox, onGroupTitleChange);*/
    tox_callback_file_chunk_request(tox, CoreFile::onFileDataCallback, nullptr);
    tox_callback_file_recv(tox, CoreFile::onFileReceiveCallback, nullptr);
    tox_callback_file_recv_chunk(tox, CoreFile::onFileRecvChunkCallback, nullptr);
    tox_callback_file_recv_control(tox, CoreFile::onFileControlCallback, nullptr);
}
...
bool Core::sendMessageWithType(uint32_t friendId, const QString& message, TOX_MESSAGE_TYPE type, ReceiptNum& receipt)
{
    ...
    receipt = ReceiptNum{tox_friend_send_message(tox.get(), friendId, type, cMessage.data(), cMessage.size(), &error)};
...
QString Core::getPeerName(const ToxPk& id) const
{
    ...
    uint8_t* cname = new uint8_t[nameSize < 128? 128 : nameSize];
