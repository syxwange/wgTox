 ...
 */
std::map<uint32_t, ToxFriendCall> CoreAV::calls;
...
CoreAV::CoreAV(std::unique_ptr<ToxAV, ToxAVDeleter> toxav) : toxav{std::move(toxav)} , coreavThread{new QThread{this}}  , iterateTimer{new QTimer{this}}
    , threadSwitchLock{}
{
    assert(coreavThread);
    assert(iterateTimer);
...
VideoSource* CoreAV::getVideoSourceFromCall(int friendNum) const
{
	...
	return  nullptr;            // it->second.getVideoSource();
}
