...
ToxFriendCall::ToxFriendCall(uint32_t FriendNum, bool VideoEnabled, CoreAV& av)  : ToxCall(VideoEnabled, av)
{
    //// register audio
    //Audio& audio = Audio::getInstance();
    //audio.subscribeInput();
...
ToxFriendCall::ToxFriendCall(ToxFriendCall &&other) noexcept   : ToxCall(std::move(other))   , alSource{other.alSource}
{
    other.alSource = 0;
}
