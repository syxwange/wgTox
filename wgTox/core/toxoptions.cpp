                                                       ...
                                                       const ICoreSettings* s)
{
    ...
    // savedata
    ////tox_options_set_savedata_type(*toxOptions, savedata.isNull() ? TOX_SAVEDATA_TYPE_NONE  : TOX_SAVEDATA_TYPE_TOX_SAVE);
	toxOptions->options->savedata_type= savedata.isNull() ? TOX_SAVEDATA_TYPE_NONE : TOX_SAVEDATA_TYPE_TOX_SAVE;
    ////tox_options_set_savedata_data(*toxOptions, reinterpret_cast<const uint8_t*>(savedata.data()), savedata.size());
	toxOptions->options->savedata_data = reinterpret_cast<const uint8_t*>(savedata.data());
	toxOptions->options->savedata_length= savedata.size();
	...
	toxOptions->options->proxy_type = TOX_PROXY_TYPE_NONE;
	toxOptions->options->proxy_port = 0;
	toxOptions->options->proxy_host = nullptr;

    if (proxyType != ICoreSettings::ProxyType::ptNone) {
        if (static_cast<uint32_t>(proxyAddr.length()) > 255)
		{
		...
		else if (!proxyAddr.isEmpty() && proxyPort > 0)
		{
			...
			{
				toxOptions->options->proxy_type = TOX_PROXY_TYPE_SOCKS5;
            } 
			else if (proxyType == ICoreSettings::ProxyType::ptHTTP)
			{
				toxOptions->options->proxy_type = TOX_PROXY_TYPE_HTTP;
            }

			toxOptions->options->proxy_host = toxOptions->getProxyAddrData();
			toxOptions->options->proxy_port =  proxyPort;
	...
	///local_discovery_enabled(没有设置）新版本没有
	toxOptions->options->udp_enabled = !forceTCP;
	toxOptions->options->ipv6_enabled = enableIPv6;
	toxOptions->options->start_port = 0;
	toxOptions->options->end_port = 0;
    return toxOptions;
}
