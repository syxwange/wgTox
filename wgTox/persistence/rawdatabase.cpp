...
QString RawDatabase::deriveKey(const QString& password)
{
	...
	////自己修改的，不知道对不对
	TOX_PASS_KEY out{};
	tox_derive_key_with_salt(reinterpret_cast<const uint8_t*>(passData.data()), static_cast<std::size_t>(passData.size()), expandConstant ,&out,nullptr);
    return QByteArray(reinterpret_cast<char*>(out.key)).toHex();
}
...
QString RawDatabase::deriveKey(const QString& password, const QByteArray& salt)
{
	...
	TOX_PASS_KEY out{};
	tox_derive_key_with_salt(reinterpret_cast<const uint8_t*>(passData.data()), static_cast<std::size_t>(passData.size()),
		reinterpret_cast<const uint8_t*>(salt.constData()),  &out, nullptr);
	return QByteArray(reinterpret_cast<char*>(out.key)).toHex();
}
