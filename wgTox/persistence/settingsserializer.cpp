...
SettingsSerializer::SettingsSerializer(QString filePath, const ToxEncrypt* passKey) :path{filePath} , passKey{passKey},group{-1} , array{-1}, arrayIndex{-1}
{
}
...
{
    auto index = std::find_if(std::begin(arrays), std::end(arrays), [=](const Array& a) { return a.name == prefix; });
